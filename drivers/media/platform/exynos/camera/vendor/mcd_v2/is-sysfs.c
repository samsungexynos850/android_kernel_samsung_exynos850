...
static struct is_rom_info *finfo[SENSOR_POSITION_MAX] = {NULL, };
static struct is_rom_info *pinfo = NULL;
...
{
	int ret = 0;
	struct device *is_vendor_dev = &sysfs_core->ischain[0].pdev->dev;

	is_sec_get_sysfs_pinfo_rear(&pinfo);
	is_sec_get_sysfs_finfo_by_position(position, &(finfo[position]));
	...
	if (force_caldata_dump || !(finfo[position]->is_caldata_read)) {
		...
		ret = is_sec_run_fw_sel(is_vendor_dev, position);
		if (ret) {
...
struct ssrm_camera_data SsrmCameraInfo[IS_VENDOR_SENSOR_COUNT];
		...
		const char *buf, size_t count)
{
	...
	case SSRM_CAMERA_INFO_CLEAR:
		for (i = 0; i < IS_VENDOR_SENSOR_COUNT; i++) { /* clear */
	...
	case SSRM_CAMERA_INFO_SET:
		for (i = 0; i < IS_VENDOR_SENSOR_COUNT; i++) { /* find empty space*/
...
static ssize_t camera_tilt_show(char *buf, int position)
{
	...
	is_sec_get_cal_buf(rom_position, &cal_buf);

	if (!is_sec_check_rom_ver(sysfs_core, rom_position)) {
	...
	if (specific->rom_cal_map_addr[rom_position]->rom_dual_cal_data2_size > 0) {
		...
		int32_t dual_tilt_dll_model_id = specific->rom_cal_map_addr[rom_position]->rom_dual_tilt_dll_model_id_addr;
		int32_t dual_tilt_dll_model_id_length = specific->rom_cal_map_addr[rom_position]->rom_dual_tilt_dll_model_id_size;
		...
		struct device_attribute *attr, char *buf)
{
	...
	struct device *is_vendor_dev = NULL;
	...
	is_vendor_dev = &sysfs_core->ischain[0].pdev->dev;

	dev_info(dev, "%s: E", __func__);
	if (force_caldata_dump == false) {
		is_vender_check_hw_init_running();
		ret = is_sec_run_fw_sel(is_vendor_dev, SENSOR_POSITION_REAR);
		if (ret) {
			err("is_sec_run_fw_sel is fail(%d)", ret);
		}
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0)
		return sprintf(buf, "UNKNOWN_UNKNOWN_IS\n");
	else
		return sprintf(buf, "%s_%s_IS\n", sensor_maker, sensor_name);
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	is_sec_set_loaded_fw(loaded_fw);
...
#ifdef CONFIG_OF
	struct is_cam_info *rear_cam_info = &(cam_infos[CAM_INFO_REAR]);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!is_sec_check_rom_ver(sysfs_core, SENSOR_POSITION_REAR)) {
		err(" NG, invalid ROM version");
		return sprintf(buf, "%s\n", "NG");
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (rom_valid) {
		...
		if (is_sec_check_rom_ver(sysfs_core, SENSOR_POSITION_FRONT)
...
static ssize_t camera_supported_cameraIds_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	const int max_supported_camera = common_cam_infos.max_supported_camera;
	char temp_buf[max_supported_camera];
	char *end = "\n";
	int i;

	for (i=0; i < max_supported_camera; i++)
	{
		...
		struct device_attribute *attr, char *buf)
{
	...
	read_from_firmware_version(SENSOR_POSITION_REAR);
	is_sec_get_cal_buf(SENSOR_POSITION_REAR, &cal_buf);

	memcpy(buf, &cal_buf[finfo[SENSOR_POSITION_REAR]->mtf_data_addr], IS_RESOLUTION_DATA_SIZE);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (specific->rom_data[SENSOR_POSITION_FRONT].rom_valid) {
		if (force_caldata_dump == false) {
			is_vender_check_hw_init_running();
			ret = is_sec_run_fw_sel(is_vendor_dev, SENSOR_POSITION_FRONT);
			if (ret) {
				err("is_sec_run_fw_sel is fail(%d)", ret);
			}
		}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (specific->rom_data[SENSOR_POSITION_FRONT].rom_valid) {
		...
		ret = is_get_sensor_data(NULL, sensor_name, SENSOR_POSITION_FRONT);
...
#ifdef CONFIG_OF
	struct is_cam_info *front_cam_info = &(cam_infos[CAM_INFO_FRONT]);
		...
		struct device_attribute *attr, char *buf)
{
	...
	read_from_firmware_version(SENSOR_POSITION_FRONT);
	is_sec_get_cal_buf(SENSOR_POSITION_FRONT, &cal_buf);

	memcpy(buf, &cal_buf[finfo[SENSOR_POSITION_FRONT]->mtf_data_addr], IS_RESOLUTION_DATA_SIZE);
	return IS_RESOLUTION_DATA_SIZE;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	read_from_firmware_version(SENSOR_POSITION_FRONT);
	is_sec_get_front_cal_buf(&cal_buf);
...
{
	int position = CAMERA_REAR2;
	char camera_info[135] = {0, };

#ifdef CONFIG_OF
	struct is_cam_info *cam_info = &(cam_infos[position]);
		...
		struct device_attribute *attr, char *buf)
{
	...
	memcpy(buf, &cal_buf[finfo[position]->mtf_data_addr], IS_RESOLUTION_DATA_SIZE);
	return IS_RESOLUTION_DATA_SIZE;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	dev_info(dev, "%s: E", __func__);
	return sprintf(buf, "%d\n", specific->sensor_id[CAMERA_FRONT2]);
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = is_get_sensor_data(sensor_maker, sensor_name, CAMERA_FRONT2);
...
#ifdef CONFIG_OF
	struct is_cam_info *front_cam_info = &(cam_infos[position]);
		...
		struct device_attribute *attr, char *buf)
{
	...
	read_from_firmware_version(CAMERA_FRONT2);
	is_sec_get_cal_buf(CAMERA_FRONT2, &cal_buf);

	memcpy(buf, &cal_buf[finfo[CAMERA_FRONT2]->mtf_data_addr], IS_RESOLUTION_DATA_SIZE);
	return IS_RESOLUTION_DATA_SIZE;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%d\n", specific->sensor_id[CAMERA_REAR4]);
}
 ...
 * rear4_hwparam   - rear4_camera_hw_param_show  / rear4_camera_hw_param_store     *****
 *                                                                                 *****
 ***************************************************************************************/
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (is_sec_is_valid_moduleid(finfo[position]->rom_module_id)) {
				    ...
				    struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "c", 1)) {
		is_sec_get_hw_param(&ec_param, SENSOR_POSITION_REAR);
				    ...
				    struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "c", 1)) {
		is_sec_get_hw_param(&ec_param, SENSOR_POSITION_FRONT);
				    ...
				    struct device_attribute *attr, const char *buf, size_t count)
{
	struct cam_hw_param *ec_param = NULL;
	if (!strncmp(buf, "c", 1)) {
		is_sec_get_hw_param(&ec_param, CAMERA_REAR2);
				    ...
				    struct device_attribute *attr, const char *buf, size_t count)
{
	struct cam_hw_param *ec_param = NULL;
	if (!strncmp(buf, "c", 1)) {
		is_sec_get_hw_param(&ec_param, SENSOR_POSITION_FRONT2);
				    ...
				    struct device_attribute *attr, const char *buf, size_t count)
{
	struct cam_hw_param *ec_param = NULL;
	if (!strncmp(buf, "c", 1)) {
		is_sec_get_hw_param(&ec_param, CAMERA_REAR3);
				    ...
				    struct device_attribute *attr, const char *buf, size_t count)
{
	struct cam_hw_param *ec_param = NULL;
	if (!strncmp(buf, "c", 1)) {
		is_sec_get_hw_param(&ec_param, CAMERA_REAR4);
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	if (check_ois_power) {
		is_ois_offset_test(sysfs_core, &raw_data_x, &raw_data_y);
		msleep(50);
...
int is_create_rear_sysfs(struct class *camera)
{
...
int is_create_front_sysfs(struct class *camera)
{
	struct class *cameraClass = camera;
...
#ifdef CAMERA_REAR2
int is_create_rear2_sysfs(void)
{
	if (IS_ERR(camera_rear_dev)) {
		printk(KERN_ERR "failed to create rear device!\n");
		return 0;
...
#ifdef CAMERA_FRONT2
int is_create_front2_sysfs(void)
{
	if (IS_ERR(camera_front_dev)) {
		printk(KERN_ERR "failed to create front device!\n");
		return 0;
...
#ifdef CAMERA_REAR3
int is_create_rear3_sysfs(void)
{
	if (IS_ERR(camera_rear_dev)) {
		printk(KERN_ERR "failed to create rear device!\n");
		return 0;
...
#ifdef CAMERA_REAR4
int is_create_rear4_sysfs(void)
{
	if (IS_ERR(camera_rear_dev)) {
		printk(KERN_ERR "failed to create rear device!\n");
		return 0;
...
#ifdef CONFIG_OIS_USE
int is_create_ois_sysfs(struct class *ois)
{
...
int is_create_sysfs(struct is_core *core)
{
	...
	is_create_rear_sysfs(camera_class);

	is_create_front_sysfs(camera_class);

#ifdef CAMERA_REAR2
	is_create_rear2_sysfs();
#endif

#ifdef CAMERA_FRONT2
	is_create_front2_sysfs();
#endif

#ifdef CAMERA_REAR3
	is_create_rear3_sysfs();
#endif
#ifdef CAMERA_REAR4
	is_create_rear4_sysfs();
#endif

#ifdef CONFIG_OIS_USE
	int is_create_ois_sysfs(camera_class);
#endif
...
void is_destroy_rear_sysfs(void)
{
...
void is_destroy_front_sysfs(void)
{
...
#ifdef CAMERA_REAR2
void is_destroy_rear2_sysfs(void)
{
...
#ifdef CAMERA_FRONT2
void is_destroy_front2_sysfs(void)
{
...
#ifdef CAMERA_REAR3
void is_destroy_rear3_sysfs(void)
{
...
#ifdef CAMERA_REAR4
void is_destroy_rear4_sysfs(void)
{
	...
	device_remove_file(camera_rear_dev, &dev_attr_rear4_dualcal);
	device_remove_file(camera_rear_dev, &dev_attr_rear4_dualcal_size);
...
{
	if (camera_rear_dev) {
		is_destroy_rear_sysfs();

#ifdef CAMERA_REAR2
		is_destroy_rear2_sysfs();
#endif
#ifdef CAMERA_REAR3
		is_destroy_rear3_sysfs();
#endif
#ifdef CAMERA_REAR4
		is_destroy_rear4_sysfs();
#endif
	}

	if (camera_front_dev) {
		is_destroy_front_sysfs();

#ifdef CAMERA_FRONT2
		is_destroy_front2_sysfs();
#endif
	}
